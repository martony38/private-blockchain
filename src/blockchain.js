...
class Blockchain {
  ...
  _addBlock(block) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (height > -1) {
        const previousBlock = await self.getBlockByHeight(height);
        if (previousBlock) {
          block.previousBlockHash = previousBlock.hash;
        } else {
          reject(new Error("Previous block cannot be found"));
        }
      }
  ...
  submitStar(address, message, signature, star) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (currentTime - time > 300) {
        reject(new Error("Ownership message is no longer valid"));
      }
      if (!bitcoinMessage.verify(message, address, signature)) {
        reject(new Error("Message signature is invalid"));
      }
      try {
        const block = await self._addBlock(
          new BlockClass.Block({ data: { owner: address, star } })
        );
        resolve(block);
      } catch (error) {
        reject(error);
  ...
  getBlockByHash(hash) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (!block) {
        reject(new Error(`Could not find block with hash ${hash}`));
      }
      resolve(block);
    });
  }
    ...
    return new Promise(async (resolve, reject) => {
      try {
        const data = await Promise.all(
          self.chain.slice(1).map(b => b.getBData())
        );
  ...
  async validateChain() {
    ...
    for (const block of this.chain) {
      ...
      if (!isValid) {
        ...
        isValid = block.previousBlockHash === (prevBlock && prevBlock.hash);
        if (!isValid) {
          errorLog.push(new Error(`block ${block.height} is not valid`));
        }
      }
